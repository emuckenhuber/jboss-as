...
public class CumulativePatchingScenariosTestCase extends AbstractPatchingTestCase {
    ...
    private File createSecondCumulativePatch(String patchID, String asVersion, final String targetAsVersion) throws Exception {
        String layerPatchID = randomString();
        File cpPatchDir = mkdir(tempDir, patchID);
        ...
        byte[] patchedAsVersionHash = null;
        for(String pathItem : CliUtilsForPatching.getResourceLoaderPathsForModule(versionModuleName, true)) {
            if(pathItem.endsWith("/.")) {
                patchedAsVersionHash =  HashUtils.hashFile(new File(pathItem.substring(0, pathItem.length() - 2)));
                break;
            }
        }
    ...
    public void testOneOffCPOneOffCP() throws Exception {
        ...
        File cpZip2 = createSecondCumulativePatch(cpPatchID2, eapWithCP, "EAP with second CP");
