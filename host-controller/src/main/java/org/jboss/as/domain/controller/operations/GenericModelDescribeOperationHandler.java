...
public class GenericModelDescribeOperationHandler implements OperationStepHandler {
    ...
    public static final GenericModelDescribeOperationHandler INSTANCE = new GenericModelDescribeOperationHandler("describe-model");
    ...
    protected GenericModelDescribeOperationHandler(final String operationName) {
    ...
    public void internalExecute(final OperationContext context, final ModelNode operation) throws OperationFailedException {
                    ...
                    public void execute(OperationContext context, ModelNode operation) throws OperationFailedException {
                        if (failureRef.get() == null) {
                            if (childRsp.hasDefined(FAILURE_DESCRIPTION)) {
                                ...
                                for (ModelNode op : childRsp.require(RESULT).asList()) {
                                    results.add(op);
                                }
                            }
                        }
        ...
        if (address.size() != 0 && model.isDefined()) {
